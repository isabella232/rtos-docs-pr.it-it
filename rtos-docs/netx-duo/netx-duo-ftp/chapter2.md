...
description: Questo capitolo contiene una descrizione dei vari problemi relativi all'installazione, alla configurazione e all'utilizzo dei servizi FTP di NetX Duo.
author: philmea
ms.author: philmea
ms.date: 07/14/2020
ms.topic: article
...
ms.locfileid: "116790225"
---
# <a name="chapter-2---installation-and-use-of-ftp"></a>Capitolo 2 - Installazione e uso di FTP

Questo capitolo contiene una descrizione dei vari problemi relativi all'installazione, alla configurazione e all'utilizzo dei servizi FTP di NetX Duo.
...
- **demo_netxduo_ftp.c** Sistema dimostrativo FTP

## <a name="netx-duo-ftp-installation"></a>Installazione FTP di NetX Duo

Per usare l'API FTP NetX Duo, l'intera distribuzione indicata in precedenza deve essere copiata nella stessa directory in cui è installato NetX Duo. Ad esempio, se NetX Duo è installato nella directory "*\threadx\arm7\green*", i file *nxd_ftp_client.h* e *nxd_ftp_client.c* devono essere copiati in questa directory per le applicazioni client FTP e i file *nxd_ftp_server.h* e *nxd_ftp_server.c* devono essere copiati in questa directory per le applicazioni server FTP.

## <a name="using-netx-duo-ftp"></a>Uso di NETX Duo FTP

L'uso dell'API FTP di NetX Duo è semplice. In pratica, il codice dell'applicazione deve includere *nxd_ftp_client.h* per le applicazioni client FTP o *nxd_ftp_server* per le applicazioni server FTP, dopo aver incluso *rispettivamente tx_api.h, fx_api.h* e *nx_api.h*, per poter usare rispettivamente ThreadX, FileX e NetX Duo. Il progetto di compilazione deve includere il codice sorgente FTP e il file dell'applicazione host e, naturalmente, i file di libreria ThreadX e NetX. Questo è tutto ciò che è necessario per usare NETX Duo FTP.

Si noti che poiché FTP usa i servizi TCP NetX Duo, è *necessario che* TCP sia abilitato nx_tcp_enable chiamata prima di usare FTP.

Si noti che la libreria NetX Duo può essere abilitata per IPv6 e supporta comunque le reti IPv4. Tuttavia, NetX Duo non può supportare IPv6 a meno che non sia abilitato. Per disabilitare l'elaborazione IPv6 in NetX Duo, il **NX_DISABLE_IPV6** deve essere definito nel file *nx_user.h* e tale file deve essere incluso nella compilazione della libreria NetX Duo definendo **NX_INCLUDE_USER_DEFINE_FILE** nel file *nx_port.h.* Per impostazione predefinita, **NX_DISABLE_IPV6** non è definito (IPv6 è abilitato). Questo è diverso dal *servizio nxd_ipv6_enable* che configura i protocolli e i servizi IPv6 nell'attività IP e richiede che NX_DISABLE_IPV6 non sia definito. 

## <a name="small-example-system-of-netx-duo-ftp"></a>Small Example System of NetX Duo FTP

Un esempio di quanto sia semplice usare NetX Duo FTP è descritto nella figura 1.1 riportata di seguito. In questo esempio vengono creati sia un server FTP che un client FTP. Entrambi i file di *inclusione FTP nxd_ftp_client.h e nxd_ftp_server.h* vengono quindi inclusi nella riga 10 e 11. Successivamente, il server FTP viene creato in "*tx_application_define*" alla riga 99. Si noti che i blocchi di controllo server FTP e client sono definiti come variabili globali alla riga 26 in precedenza.

Questa demo illustra come usare le funzioni duo disponibili in NETX Duo FTP e i servizi FTP legacy limitati IPv4. Per usare le funzioni IPv6, la demo definisce USE_IPV6 nella riga 16

Alla riga 162 il server FTP viene creato con ***nxd_ftp_server_create** _ se l'applicazione host definisce USE_IPV6 che supporta sia IPv4 che IPv6. In caso contrario, il server FTP viene creato con _ *_nx_ftp_server_create_** alla riga 166 con il servizio limitato IPv4. Si noti che la funzione 'duo' usa argomenti della funzione di accesso e disconnessione diversi rispetto al servizio IPv4, entrambi definiti nella parte inferiore del file nelle righe 534 -568.

Il server FTP deve quindi stabilire il proprio indirizzo IPv6 (globale e locale di collegamento) con NetX Duo, a partire dalla riga 466 nella funzione di immissione del thread del server FTP. Il server FTP viene quindi avviato alla riga 518 ed è pronto per le richieste client FTP.

Il client FTP viene creato nella riga 316 ed è in esecuzione lo stesso processo del server FTP per ottenere l'attività IP client FTP IPv6 abilitata e i relativi indirizzi IPv6 convalidati a partire dalle righe 263-313.

Il client si connette quindi al server FTP usando ***nxd_ftp_client_connect** _ nella riga 334 se ha definito USE_IPV6 o la riga 340 se usa il servizio limitato IPv4 _*_nx_ftp_client_connect_**. Nel corso della funzione del thread del client FTP, scrive un file nel server FTP e lo legge di nuovo prima della disconnessione.
...
**Figura 1.1 Esempio di NETX Duo FTP**

## <a name="configuration-options"></a>Opzioni di configurazione

Sono disponibili diverse opzioni di configurazione per la creazione di FTP NetX e NetX Duo FTP. I valori predefiniti sono elencati, ma ogni definizione può essere impostata da

prima dell'inclusione del file di intestazione FTP NetX Duo specificato. Se non viene specificato alcun file di intestazione, l'opzione è disponibile sia in *nxd_ftp_client.h che in nxd_ftp_server.h*. L'elenco seguente descrive ognuno in dettaglio:
...
- **NX_FTP_SERVER_TIMEOUT** Specifica il numero di tick ThreadX per cui i servizi interni verranno sospesi. Il valore predefinito è impostato su 1 secondo (1 * NX_IP_PERIODIC_RATE).
- **NX_FTP_ACTIVITY_TIMEOUT** Specifica il numero di secondi per cui viene mantenuta una connessione client se non è presente alcuna attività. Il valore predefinito è impostato su 240.
- **NX_FTP_TIMEOUT_PERIOD** Specifica gli intervalli in secondi quando il server verifica l'attività client. Il valore predefinito è impostato su 60.
...
- **NX_FTP_SERVER_TRANSMIT_QUEUE_DEPTH** Specifica la profondità massima dei pacchetti di trasmissione in coda nel socket del server. Il valore predefinito è 20.
...
- **NX_FTP_NO_FILEX** Definita, questa opzione fornisce uno stub per le dipendenze FileX. Il client FTP funzionerà senza alcuna modifica se questa opzione è definita. Il server FTP dovrà essere modificato o l'utente dovrà creare una serie di servizi FileX per funzionare correttamente.
- **NX_FTP_CONTROL_TOS** Tipo di servizio necessario per le richieste di controllo FTP. Per impostazione predefinita, questo valore viene definito come NX_IP_NORMAL per indicare il normale servizio di pacchetti IP.
- **NX_FTP_DATA_TOS** Tipo di servizio necessario per le richieste di dati FTP. Per impostazione predefinita, questo valore viene definito come NX_IP_NORMAL per indicare il normale servizio di pacchetti IP.
- **NX_FTP_FRAGMENT_OPTION** Frammento abilitato per le richieste FTP. Per impostazione predefinita, questo valore è NX_DONT_FRAGMENT per disabilitare la frammentazione TCP FTP.
- **NX_FTP_CONTROL_WINDOW_SIZE** Dimensioni della finestra del socket di controllo TCP. Per impostazione predefinita, questo valore è 400 byte.
- **NX_FTP_DATA_WINDOW_SIZE** Dimensioni della finestra del socket di dati TCP. Per impostazione predefinita, questo valore è 2048 byte.
- **NX_FTP_TIME_TO_LIVE** Specifica il numero di router che il pacchetto può passare prima di essere eliminato. Il valore predefinito è impostato su 0x80.
