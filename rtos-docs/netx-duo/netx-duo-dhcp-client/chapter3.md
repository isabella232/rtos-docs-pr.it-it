---
title: Capitolo 3 - Descrizione dei Azure RTOS client DHCP NetX Duo
description: Questo capitolo contiene una descrizione di tutti Azure RTOS client DHCP NetX Duo (elencati di seguito) in ordine alfabetico.
author: philmea
ms.author: philmea
ms.date: 06/04/2020
ms.topic: article
...
ms.locfileid: "116790344"
---
# <a name="chapter-3---description-of-azure-rtos-netx-duo-dhcp-client-services"></a>Capitolo 3 - Descrizione dei Azure RTOS client DHCP NetX Duo

Questo capitolo contiene una descrizione di tutti Azure RTOS client DHCP NetX Duo (elencati di seguito) in ordine alfabetico.
...
- **nx_dhcp_clear_broadcast_flag:** Cancella *flag di trasmissione nei messaggi client*
...
- **nx_dhcp_reinitialize:** Cancella *i parametri di rete del client DHCP*
...
- **nx_dhcp_state_change_notify**: impostare *la funzione di callback quando cambia lo stato DHCP*
- **nx_dhcp_user_option_retrieve**: *recupera l'opzione DHCP specificata*
- **nx_dhcp_user_option_convert**: *Convertire quattro byte in ULONG*

Servizi client DHCP specifici dell'interfaccia:
 
- **nx_dhcp_interface_clear_broadcast_flag:** Cancella *flag di trasmissione nei messaggi client sull'interfaccia specificata*
- **nx_dhcp_interface_enable**: abilita *l'interfaccia per l'esecuzione di DHCP nell'interfaccia specificata*
- **nx_dhcp_interface_disable**: *disabilita l'interfaccia per l'esecuzione di DHCP nell'interfaccia specificata*
...
- **nx_dhcp_interface_start**: avvia *l'elaborazione del client DHCP sull'interfaccia specificata*
- **nx_dhcp_interface_stop**: *arresta l'elaborazione del client DHCP nell'interfaccia specificata*
...
- **nx_dhcp_interface_user_option_retrieve**: *recupera l'opzione DHCP specificata nell'interfaccia specificata*

Servizi client DHCP se NX_DHCP_CLIENT_RESORE_STATE è definito:
...
- **nx_dhcp_client_update_time_remaining**: *aggiorna il tempo rimanente nello stato DHCP corrente*

Interfaccia Servizi client DHCP specifici se NX_DHCP_CLIENT_RESORE_STATE è definito:

- **nx_dhcp_client_interface_get_record**: *creare un record dello stato del client DHCP nell'interfaccia specificata*
- **nx_dhcp_client_interface_restore_record**: *Ripristina un record salvato in precedenza nel client DHCP nell'interfaccia specificata*
- **nx_dhcp_client_interface_update_time_remaining**: *aggiorna il tempo rimanente nello stato DHCP corrente sull'interfaccia specificata*
...
Questo servizio crea un'istanza DHCP per l'istanza IP creata in precedenza. Per impostazione predefinita, l'interfaccia primaria è abilitata per l'esecuzione di DHCP. L'input del nome, anche se non viene usato nell'implementazione NetX Duo del client DHCP, deve seguire i criteri RFC 1035 per i nomi host. La lunghezza totale non deve superare i 255 caratteri, le etichette separate da punti devono iniziare con una lettera e terminare con una lettera o un numero e possono contenere trattini ma nessun altro carattere non alfanumerico.

Se l'applicazione desidera eseguire DHCP in un'altra interfaccia registrata con l'istanza IP ( *usando nx_ip_interface_attach*), l'applicazione può chiamare *nx_dhcp_set_interface_index* per eseguire DHCP solo su tale interfaccia oppure *nx_dhcp_interface_enable* per eseguire DHCP anche su tale interfaccia. Per altri dettagli, vedere la descrizione di questi servizi.

>[!NOTE]
> L'applicazione deve assicurarsi che il payload del pool di pacchetti del client DHCP sia in grado di supportare le dimensioni minime dei messaggi DHCP specificate nella sezione 2 rfc 2131 (548 byte di dati dei messaggi DHCP più intestazioni udp, IP e frame di rete fisica).
...
Inoltre, deve essere disponibile un 'record' dell'interfaccia client DHCP per aggiungere questa interfaccia all'elenco di interfacce abilitate. Per impostazione NX_DHCP_CLIENT_MAX_RECORDS è definito su 1. Impostare questa opzione sul numero massimo di interfacce previste per l'esecuzione simultanea del client DHCP. In genere NX_DHCP_CLIENT_MAX_RECORDS sarà uguale NX_MAX_PHYSICAL_INTERFACES; Tuttavia, se un dispositivo ha più interfacce fisiche di quelle previste per l'esecuzione di Client DHCP, può risparmiare memoria impostando NX_DHCP_CLIENT_MAX_RECORDS su un valore inferiore a tale numero. Non esiste un mapping uno-a-uno di interfacce fisiche con record di interfaccia client DHCP.

La differenza tra questo servizio *e nx_dhcp_set_interface_index* è che quest'ultimo imposta una sola interfaccia per eseguire DHCP, mentre questo servizio aggiunge semplicemente l'interfaccia specificata all'elenco di interfacce client abilitate per DHCP.

Per disabilitare un'interfaccia per DHCP, l'applicazione può chiamare

*nx_dhcp_interface_disable* servizio.
...
- **NX_DHCP_INTERFACE_ALREADY_ENABLED:** interfaccia (0xA3) abilitata per DHCP
- NX_PTR_ERROR: (0x16) Puntatore DHCP o IP non valido
...
- **NX_SUCCESS:**(0x00) Creazione DHCP riuscita
- **NX_DHCP_INTERFACE_NOT_ENABLED:** Interfaccia (0xA4) non abilitata per DHCP
- NX_PTR_ERROR: (0x16) Puntatore DHCP o IP non valido
...
- **NX_TRUE:** il flag broadcast viene cancellato (richiesta di risposta unicast)
- **NX_FALSE:** flag broadcast impostato (richiesta di risposta broadcast)
...
- **NX_SUCCESS:**(0x00) Flag Impostato correttamente
- NX_PTR_ERROR: (0x16) Puntatore DHCP o IP non valido
...
- **NX_SUCCESS:**(0x00) Successfully sent force renew (Invio forzato del rinnovo).
- **NX_DHCP_NOT_BOUND:** indirizzo IP del client (0x94) non associato.  
- NX_PTR_ERROR: (0x16) Puntatore DHCP non valido.
- NX_CALLER_ERROR: (0x11) Chiamante del servizio non valido.
...
>[!NOTE] 
> È consigliabile usare i valori predefiniti per il payload del pool di pacchetti client DHCP, definiti come NX_DHCP_PACKET_PAYLOAD in *nxd_dhcp_client.h* durante la creazione del pool di pacchetti.
...
- NX_PTR_ERROR: (0x16) Puntatore DHCP non valido
- NX_DHCP_INVALID_PAYLOAD: (0x9C) Il payload è troppo piccolo
...
Questo servizio imposta l'indirizzo IP per il client DHCP da richiedere al server DHCP nella prima interfaccia abilitata per DHCP nel record client DHCP. Se il *flag skip_discover_message* impostato, il client DHCP ignora il messaggio Di individuazione e invia un messaggio di richiesta.

Per impostare la richiesta di un indirizzo IP specifico per i messaggi DHCP in un'interfaccia specifica, usare il nx_dhcp_interface_request_client_ip *servizio.*

### <a name="input-parameters"></a>Parametri di input

- **dhcp_ptr:** puntatore al blocco di controllo DHCP.  
- **client_ip_address**: indirizzo IP da richiedere al server DHCP
- **skip_discover_message**: 
    - Se true, il client DHCP invia il messaggio di richiesta
...
Questo servizio imposta l'indirizzo IP per il client DHCP da richiedere al server DHCP nell'interfaccia specificata, se tale interfaccia è abilitata per DHCP (vedere *nx_dhcp_interface_enable*). Se il *flag skip_discover_message* impostato, il client DHCP ignora il messaggio Di individuazione e invia un messaggio di richiesta.
...
- **NX_SUCCESS:**(0x00) L'indirizzo IP richiesto è impostato.
- **NX_DHCP_INTERFACE_NOT_ENABLED:**(0xA4) Interfaccia non abilitata per DHCP
...
Per reinizializzare il client DHCP in un'interfaccia specifica quando sono abilitate più interfacce per DHCP, usare il nx_dhcp_interface_reinitialize *dhcp.*
...
- **NX_SUCCESS:**(0x00) DHCP reinizializzato correttamente 
- NX_PTR_ERROR: (0x16) Puntatore DHCP non valido
...
Cancellare i parametri di rete client DHCP nell'interfaccia specificata 
...
- **NX_SUCCESS:**(0x00) L'interfaccia è stata reinizializzata correttamente
- **NX_DHCP_INTERFACE_NOT_ENABLED:**(0xA4) Interfaccia non abilitata per DHCP
- NX_PTR_ERROR: (0x16) Puntatore DHCP non valido
- NX_CALLER_ERROR: (0x11) Chiamante non valido di questo servizio.
- NX_INVALID_INTERFACE: (0x4C) Interfaccia di rete non valida
...
Per rilasciare un indirizzo al server DHCP in un'interfaccia specifica, usare il *nx_dhcp_interface_release* locale
...
- **NX_DHCP_NOT_BOUND**: (0x94) L'indirizzo IP non è stato lease, quindi non può essere rilasciato.
- NX_PTR_ERROR: (0x16) Puntatore DHCP non valido.
- NX_CALLER_ERROR: (0x11) Chiamante non valido di questo servizio.
...
Questo servizio rifiuta un indirizzo IP assegnato in lease dal server DHCP in tutte le interfacce abilitate per DHCP. Se NX_DHCP_CLIENT_ SEND_ ARP_PROBE definito, il client DHCP invierà un messaggio DECLINE se rileva che l'indirizzo IP è già in uso. Per altre informazioni sulla configurazione dei probe ARP nel client DHCP NetX Duo, vedere Probe **ARP** nel capitolo 1.
...
Questo servizio invia il messaggio DECLINE al server per rifiutare un indirizzo IP assegnato dal server DHCP. Reinizializza anche il client DHCP. Vedere *nx_dhcp_decline* per altri dettagli.
...
Questo servizio invia il messaggio DHCP specificato al server DHCP nella prima interfaccia abilitata per DHCP trovata nel record client DHCP. Per inviare un messaggio RELEASE o DECLINE, l'applicazione deve usare rispettivamente i servizi *nx_dhcp[_interface]_release*() *o nx_dhcp_interface_decline().*

Il client DHCP deve essere avviato per usare questo servizio, ad eccezione dell'invio del INFORM_REQUEST messaggio.

>[!NOTE]
> Questo servizio non è destinato all'applicazione host per "guidare" la macchina a stati del client DHCP.

### <a name="input-parameters"></a>Parametri di input

- **dhcp_ptr:** puntatore al blocco di controllo DHCP.  
- **dhcp_message_type:** richiesta di messaggio (definita in *nxd_dhcp_client.h)*

### <a name="return-values"></a>Valori restituiti

- **NX_SUCCESS:** messaggio DHCP (0x00) inviato  
...
Inviare un messaggio DHCP al server in un'interfaccia specifica
...
Il client DHCP deve essere avviato per usare questo servizio, ad eccezione dell'invio del tipo di messaggio DHCP INFORM REQUEST.
...
- **NX_INVALID_INTERFACE**: (0x4C) Interfaccia di rete non valida
- **NX_DHCP_INTERFACE_ALREADY_ENABLED:**(0xA3) Interfaccia abilitata per DHCP
...
- **dhcp_ptr:** puntatore all'istanza DHCP creata in precedenza.
- **dhcp_state_change_notify:** Puntatore a funzione di callback di modifica dello stato
...
Questo servizio recupera l'opzione DHCP specificata dal buffer delle opzioni DHCP nella prima interfaccia abilitata per DHCP trovata nel record client DHCP. In caso di esito positivo, i dati dell'opzione vengono copiati nel buffer specificato.

### <a name="input-parameters"></a>Parametri di input

- **dhcp_ptr**: puntatore a un'istanza DHCP creata in precedenza.  
- **request_option:** opzione DHCP, come specificato dalle RFC. Vedere l'NX_DHCP_OPTION in *nxd_dhcp_client.h.*
...
- **NX_DHCP_PARSE_ERROR:** opzione (0x97) non trovata nella risposta del server.
- NX_PTR_ERROR: (0x16) Puntatore di input non valido.
- NX_CALLER_ERROR: (0x11) Chiamante del servizio non valido.
...
- **request_option:** opzione DHCP, come specificato dalle RFC. Vedere l'NX_DHCP_OPTION seguente: in *nxd_dhcp_client.h*.  
- **destination_ptr**: puntatore alla destinazione per la stringa di risposta.  
- **destination_size**: puntatore alla dimensione della destinazione e, al ritorno, alla destinazione in cui inserire il numero di byte restituiti.
...
- **NX_SUCCESS**: (0x00) Recupero dell'opzione riuscito.  
- **NX_DHCP_NOT_BOUND:** indirizzo IP (0x94) non assegnato
- **NX_DHCP_DEST_TO_SMALL:**(0x95) Buffer troppo piccolo
- **NX_DHCP_PARSE_ERROR:** opzione DHCP (0x97) non trovata nella risposta del server.
- NX_PTR_ERROR: (0x16) Puntatore DHCP non valido.
- NX_CALLER_ERROR: (0x11) Chiamante del servizio non valido.
- NX_INVALID_INTERFACE: (0x4C) Interfaccia di rete non valida
