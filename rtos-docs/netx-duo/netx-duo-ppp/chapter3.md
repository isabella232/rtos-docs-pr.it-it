...
---
# <a name="chapter-3---description-of-azure-rtos-netx-duo-point-to-point-protocol-ppp-services"></a>Capitolo 3-Descrizione dei servizi di Azure RTO NetX Duo Point-to-Point Protocol (PPP)

Questo capitolo contiene una descrizione di tutti i servizi PPP di Azure RTO NetX Duo (elencati di seguito) in ordine alfabetico.
...
Questo servizio crea un'istanza di PPP per l'istanza IP di NetX specificata. Questa funzione deve essere chiamata prima della creazione dell'istanza IP di NetX.
...
Questo servizio recupera l'indirizzo IP DNS fornito dal peer nell'handshake protocollo IPCP. Se non è stato fornito alcun indirizzo IP dal peer, viene restituito un indirizzo IP 0.
...
Questo servizio configura gli indirizzi IP locali e peer per l'utilizzo in Internet Protocol Control Protocol (protocollo IPCP). Deve essere chiamato per l'istanza di PPP che dispone di indirizzi IP validi per se stesso e per l'altro peer.
...
Questo servizio invia una richiesta LCP e imposta un flag che il dispositivo PPP è in attesa di una risposta echo. L'opzione Wait è principalmente per la chiamata *nx_packet_allocate* . Il servizio viene restituito non appena viene inviata la richiesta. Non attende una risposta. 
