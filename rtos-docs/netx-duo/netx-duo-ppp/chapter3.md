---
title: Capitolo 3 - Descrizione dei Azure RTOS NetX Duo Point-to-Point Protocol (PPP)
description: Questo capitolo contiene una descrizione di tutti i Azure RTOS PPP di NetX Duo (elencati di seguito) in ordine alfabetico.
author: philmea
ms.author: philmea
ms.date: 06/04/2020
ms.topic: article
...
ms.locfileid: "116798079"
---
# <a name="chapter-3---description-of-azure-rtos-netx-duo-point-to-point-protocol-ppp-services"></a>Capitolo 3 - Descrizione dei Azure RTOS NetX Duo Point-to-Point Protocol (PPP)

Questo capitolo contiene una descrizione di tutti i Azure RTOS PPP di NetX Duo (elencati di seguito) in ordine alfabetico.
...
- **nx_ppp_byte_receive**: *Ricevere un byte dall'ISR seriale*
...
- **nx_ppp_dns_address_get:** Ottenere *l'indirizzo IP del server DNS*
...
- **nx_ppp_link_down_notify:** *Inviare una notifica all'applicazione al collegamento*
...
Ricevere un byte dall'ISR seriale
...
- **NX_SUCCESS**: (0x00) Ricezione di byte PPP riuscita.
...
Questo servizio avvia una richiesta CHAP dopo che la connessione PPP è già in esecuzione. In questo modo l'applicazione può verificare periodicamente l'autenticità della connessione. Se la richiesta di verifica ha esito negativo, il collegamento PPP viene chiuso.
...
Se vengono specificati **i puntatori get_challenge_values**_" e "_ * _get_verification_values_**", CHAP è richiesto da questa istanza PPP. In caso contrario, CHAP risponde solo alle richieste di richiesta di richiesta del peer.

Le funzioni di callback necessarie fanno riferimento a diversi elementi di dati riportati di seguito. Il segreto degli *elementi*  di *dati,* il nome e il sistema devono essere stringhe con terminazione NULL con una dimensione massima di NX_PPP_NAME_SIZE-1. *L'elemento rand_value* deve essere una stringa con terminazione NULL con una dimensione massima di NX_PPP_VALUE_SIZE-1. *L'ID dell'elemento* dati è un tipo di carattere senza segno semplice.

>[!NOTE]
> Questa funzione deve essere chiamata dopo *nx_ppp_create* ma prima nx_ip_create o *nx_ip_interface_attach*.

### <a name="input-parameters"></a>Parametri di input

- **ppp_ptr:** puntatore al blocco di controllo PPP.
- **get_challenge_values:** puntatore alla funzione dell'applicazione per recuperare i valori usati per la sfida. Si noti che *i rand_value*, *id* e *secret* devono essere copiati nelle destinazioni fornite.
...
- **get_verification_values:** puntatore alla funzione dell'applicazione che recupera i valori usati per verificare la risposta alla richiesta di verifica. Si noti che *i valori**di* sistema , name e *secret* devono essere copiati nelle destinazioni fornite.

### <a name="return-values"></a>Valori restituiti

- **NX_SUCCESS:**(0x00) Autenticazione CHAP PPP riuscita
- **NX_NOT_IMPLEMENTED:** la logica CHAP (0x80) è stata disabilitata tramite NX_PPP_DISABLE_CHAP.
- NX_PTR_ERROR: (0x07) Puntatore PPP o puntatore a funzione di callback non valido. Si noti che *se get_challenge_values* specificato, è necessario *specificare get_verification_values* funzione.
- NX_CALLER_ERROR: (0x11) Chiamante del servizio non valido.
...
Questo servizio crea un'istanza PPP per l'istanza IP NetX specificata. Questa funzione deve essere chiamata prima di creare l'istanza IP NetX.

>[!NOTE]
> È in genere consigliabile creare il thread IP NetX con una priorità più alta rispetto alla priorità del thread PPP. Per altre informazioni su come specificare la priorità del thread *IP,* vedere il servizio di nx_ip_create ip.

### <a name="input-parameters"></a>Parametri di input

- **ppp_ptr:** puntatore al blocco di controllo PPP.
- **name:** nome dell'istanza PPP.
...
- **ppp_invalid_packet_handler:** puntatore a funzione al gestore dell'applicazione per tutti i pacchetti non PPP. Il PPP NetX in genere chiama questa routine durante l'inizializzazione. Qui l'applicazione può rispondere ai comandi del modem o, nel caso di Windows XP, l'applicazione PPP NetX può avviare PPP rispondendo con "CLIENT SERVER" al "CLIENT" iniziale inviato da Windows XP.
- **ppp_byte_send:** puntatore a funzione alla routine di output dei byte seriali dell'applicazione.


### <a name="return-values"></a>Valori restituiti

- **NX_SUCCESS:**(0x00) Creazione PPP riuscita.
- NX_PTR_ERROR: (0x07) Puntatore a funzione PPP, IP o di output di byte non valido.
- NX_CALLER_ERROR: (0x11) Chiamante del servizio non valido.
...
Ottenere l'indirizzo IP del server DNS
...
Questo servizio recupera l'indirizzo IP DNS fornito dal peer nell'handshake IPCP. Se il peer non ha fornito alcun indirizzo IP, viene restituito un indirizzo IP pari a 0.

### <a name="input-parameters"></a>Parametri di input

- **ppp_ptr:** puntatore al blocco di controllo PPP.
- **dns_address_ptr:** destinazione per l'indirizzo del server DNS
...
- **NX_SUCCESS:**(0x00) Set di indirizzi DNS riusciti.
- **NX_PPP_NOT_ESTABLISHED:**(0xB5) PPP non ha completato la negoziazione con il peer.
- NX_PTR_ERROR: (0x07) Puntatore PPP non valido.
...
Questo servizio imposta l'indirizzo IP del server DNS secondario. Se il peer invia una richiesta di opzione server DNS secondaria nello stato IPCP, questo host fornirà le informazioni.
...
- **ppp_ptr:** puntatore al blocco di controllo PPP.
- **index_ptr:** destinazione per l'indice di interfaccia
...
Questo servizio configura gli indirizzi IP locali e peer da usare in IPCP (Internet Protocol Control Protocol). Deve essere chiamato per l'istanza PPP che dispone di indirizzi IP validi per se stesso e per l'altro peer.
...
Questo servizio registra il callback di notifica di collegamento verso il basso dell'applicazione con l'istanza PPP specificata. Se non NULL, la funzione di callback di collegamento verso il basso dell'applicazione viene chiamata ogni volta che il collegamento diventa in esecuzione.

### <a name="input-parameters"></a>Parametri di input

- **ppp_ptr:** puntatore al blocco di controllo PPP.
- **link_down_callback:** puntatore a funzione di notifica di collegamento verso il basso dell'applicazione. Se NULL, la notifica di collegamento è disabilitata.

### <a name="return-values"></a>Valori restituiti

- **NX_SUCCESS:**(0x00) Registrazione callback di notifica collegamento riuscito.
- NX_PTR_ERROR: (0x07) Puntatore PPP non valido.
...
- **NX_SUCCESS:**(0x00) Collegamento riuscito della registrazione callback di notifica.
- NX_PTR_ERROR: (0x07) Puntatore PPP non valido.
...
Questo servizio registra il callback di notifica nak di autenticazione dell'applicazione con l'istanza PPP specificata. Se non NULL, questa funzione di callback viene chiamata ogni volta che l'istanza PPP riceve un NAK durante l'autenticazione.
...
- **NX_SUCCESS:**(0x00) Registrazione callback di notifica riuscita.
- NX_PTR_ERROR: (0x07) Puntatore PPP non valido.
...
Le funzioni di callback necessarie fanno riferimento a diversi elementi di dati riportati di seguito. Si prevede che *il nome* dell'elemento dati sia una stringa con terminazione NULL con una dimensione massima di NX_PPP_NAME_SIZE-1. Si prevede anche *che la password* dell'elemento dati sia una stringa con terminazione NULL con una dimensione massima di NX_PPP_PASSWORD_SIZE-1.
...
- **NX_SUCCESS:**(0x00) PPP abilitato correttamente.
...
Questo servizio invia una richiesta LCP e imposta un flag che indica che il dispositivo PPP è in attesa di una risposta echo. L'opzione wait è principalmente per la *nx_packet_allocate* chiamata. Il servizio viene restituito non appena viene inviata la richiesta. Non attende una risposta. 

Quando viene ricevuta una risposta echo corrispondente, l'attività del thread PPP cancella il flag. Il dispositivo PPP deve aver completato la parte LCP della negoziazione PPP.
...
- **NX_SUCCESS**: (0x00) Richiesta echo inviata con esito positivo.
...
Questo servizio riavvia l'elaborazione PPP. Viene in genere chiamato quando il collegamento deve essere stabilito nuovamente da un callback di collegamento verso il basso o da un messaggio modem non PPP che indica che la comunicazione è stata persa.
...
Questo servizio avvia l'elaborazione PPP. Viene in genere chiamato dopo nx_ppp_stop() .
...
- **NX_PPP_ALREADY_STARTED:** PPP (0xb9) già avviato.
- NX_PTR_ERROR: (0x07) Puntatore PPP non valido.
- NX_CALLER_ERROR: (0x11) Chiamante del servizio non valido.
...
- **ppp_ptr:** puntatore al blocco di controllo PPP.
- **status_ptr**: destinazione dello stato PPP, i valori di stato possibili sono i seguenti:
...
>[!NOTE]
> Lo stato è valido solo se l'API restituisce NX_SUCCESS. Inoltre, se viene restituito uno dei valori di stato *_FAILED, l'elaborazione PPP viene arrestata in modo efficace fino a quando non viene riavviata nuovamente dall'applicazione.
