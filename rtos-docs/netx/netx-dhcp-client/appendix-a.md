---
title: Appendice A - Descrizione della funzionalità Stato di ripristino
description: L'Azure RTOS di configurazione del client NetX DHDP, NX_DHCP_CLIENT_RESTORE_STATE, consente a un sistema di ripristinare un record client DHCP creato in precedenza in uno stato associato tra un riavvio del sistema e l'altro.
author: philmea
ms.author: philmea
ms.date: 06/04/2020
ms.topic: article
...
ms.locfileid: "116801241"
---
# <a name="appendix-a---description-of-the-restore-state-feature"></a>Appendice A - Descrizione della funzionalità Stato di ripristino

L'Azure RTOS di configurazione del client NetX DHDP, NX_DHCP_CLIENT_RESTORE_STATE, consente a un sistema di ripristinare un record client DHCP creato in precedenza in uno stato associato tra un riavvio del sistema e l'altro.
...
Aggiornare il tempo rimanente nel lease del client DHCP
...
Questo servizio aggiorna il tempo rimanente nel lease dell'indirizzo IP del client DHCP con l'input time_elapsed sulla prima interfaccia abilitata per DHCP disponibile nell'istanza del client DHCP. L'applicazione deve sospendere il thread client DHCP prima di usare questo servizio *usando nx_dhcp_suspend*. Dopo aver chiamato questo servizio, l'applicazione può riprendere il thread client DHCP *chiamando nx_dhcp_resume*.
...
> [!NOTE]
> Questo servizio non deve essere usato con nx_dhcp_client_get_record *e* *nx_dhcp_client_restore_record* descritto in precedenza). Questi servizi sono descritti in precedenza in questa sezione.

### <a name="input-parameters"></a>Parametri di input

- **dhcp_ptr** Puntatore al client DHCP

- **time_elapsed** Tempo da sottrarre dal tempo rimanente nel lease dell'indirizzo IP
...
Aggiornare il tempo rimanente nel lease del client DHCP nell'interfaccia specificata
...
Questo servizio aggiorna il tempo rimanente nel lease dell'indirizzo IP del client DHCP time_elapsed input sull'interfaccia specificata se tale interfaccia è abilitata per DHCP. L'applicazione deve sospendere il thread client DHCP prima di usare questo servizio *usando nx_dhcp_suspend*. Dopo aver chiamato questo servizio, l'applicazione può riprendere il thread client DHCP *chiamando nx_dhcp_resume*. Si noti che la sospensione e la ripresa del thread client DHCP si applicano a tutte le interfacce abilitate per DHCP.
...
Sospendere il thread client DHCP
...
Questo servizio sospende il thread client DHCP corrente. Si noti *che, nx_dhcp_stop,* lo stato del client DHCP non viene modificato quando viene chiamato questo servizio.

Questo servizio sospende DHCP in esecuzione su tutte le interfacce abilitate per DHCP.

Per aggiornare lo stato del client DHCP con il tempo trascorso mentre il client DHCP è sospeso, vedere il nx_dhcp_client_update_time_remaining *descritto* in precedenza. Per riprendere un thread client DHCP sospeso, l'applicazione deve chiamare *nx_dhcp_resume*.
...
- **NX_SUCCESS** (0x0) Il thread client viene sospeso
...
- **NX_SUCCESS** (0x0) Il thread client viene ripreso
